...
`(apply-action env (fn [state-map] state-map))`:: use a fn of state-map to apply some mutation helper via a SM env. The return value of the function will become the new state in the env and will be applied when the handler returns.
 `(get-active-state this asm-id)`:: Read the “current state name” from an active state machine while in a UI component (e.g. via `this`).
