...
;; TASK: real impl is like mutations, so we can use symbols, though instead of a multimethod (which does not DCE well)
;; we can actually intern vars and push their symbols into a central registry. creating a "derived" machine would just
;; copy over the old def as the base for the new one....e.g. deep-merge the maps.
(def registry (atom {}))
...
(Defn new-asm
  ...
  - `instance-id` is a user-generated unique ID for the instance of the asm. This allows more than one
    instance of the same state machine definition to be active at the same time on the UI.
  - `actors` is a map from actor name to an ident.
...
(Defn activate
  ...
  [env state-id]
  [::env ::state-id => ::env]
  (log/debug "Moving to new state " state-id)
...
(Defn resolve-alias
  "Looks up the given k in the alias map. If the k isn't an alias, it is returned unmodified."
  [env k]
  [::env any? => any?]
  (let [{:keys [aliases]} (lookup-state-machine env)]
    (get aliases k k)))
...
(Defn actor-path
  ...
  ([env actor-name k]
    ...
    (if-let [ident (actor->ident env actor-name)]
      (conj ident k)
      (do
        (log/error "Failed to find actor on asm " (::asm-id env) ". The following actor has no ident: " actor-name)
        nil))))

(Defn get-actor
  "Returns the *normalized* table entry from Fulcro state for the given actor."
  [{::keys [state-map] :as env} actor-name]
  [::env ::actor-name => (s/nilable map?)]
  (let [path (actor-path env actor-name)]
    (when (futil/ident? path) (get-in state-map path))))
...
(Defn alias-value
  ...
  [::env keyword? => any?]
  (let [[actor & subpath] (lookup-state-machine-field env [::aliases alias])
...
(Defn set-aliased-value
  ...
  (let [[actor & subpath] (lookup-state-machine-field env [::aliases alias])
        base-path (actor->ident env actor)
        real-path (into base-path subpath)]
    ;;TASK: Warnings if real-path is bad
...
(defn trigger-state-machine-event!
  ...
  (log/debug "Triggering event " params)
  (let [sm-env        (cond-> {::state-map @state
    ...
    (when-let [new-fulcro-state (some-> (::state-map final-env)
      ...
      (log/debug "Updating fulcro state")
...
(defmutation trigger-state-machine-event
  ...
  (action [{:keys [reconciler] :as env}]
    ...
    (let [to-refresh (trigger-state-machine-event! env params)]
      ...
      (fulcro.client.impl.protocols/queue! reconciler to-refresh))
    true))
...
(defn begin!
  ...
  actors - A map of actor-names -> Their the idents of the real components that will represent them in the UI."
